...
double SPH_Iterative::compute_density(shared_ptr <Particle> p) {
    double result = p->rho / this->rho_zero;

    return result;
}

double SPH_Iterative::compute_pressure(shared_ptr <Particle> p) {
...
void SPH_Iterative::non_recurrent_part() {
    ...
    for (int i = 0; i < this->g->particles.size(); i++) {
        ...
        p.force_viscosity = Calculus.computeLaplaceSeed(&p, this->w_kernel);
...
void SPH_Iterative::recurrent_part() {
    ...
    do {
        for (int i = 0; i < this->g->particles.size(); i++) {
            shared_ptr <Particle> p = this->modified_particles[i];
            p->rho = compute_density(p);
            p->pressure = compute_pressure(p);
        }
        ...
        for (int i = 0; i < this->g->particles.size(); i++) {
            ...
            Vect temp = Calculus.ComputeGradientPressure(&p, this->w_kernel);
            temp *= (-p.w / p.rho);
...
SPH_Iterative::SPH_Iterative(Grid *g, double viscosity, double num, double smoothingDistance, double delta_t,
                             double rho_zero, double k) {
    this->g = g;
